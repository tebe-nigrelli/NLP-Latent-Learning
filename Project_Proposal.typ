...
Encoder-Decoder models based on the transformer are capable of performing a wide range of tasks by first mapping tokens to a sequence of encoder hidden states (embeddings), before remapping them autoregressively to a new sequence. Without additional inductive biases, embedding spaces suffer from a lack of interpretability, despite having a complete encoding of information. We investigate whether such embeddings can be refined and disentangled by a Variational Autoencoder (VAE). To do so, we train the VAE to reproduce the original sequence through an information bottleneck. We use a factor-VAE architecture to decorrelate the latent dimensions, which helps smooth the representation, and we include task-reserved dimensions for performing emotion-translating tasks. Particularly, we use an attention pooling component to learn a two-way mapping between some dimensions of the latents and the emotion labels, producing an emotion-translating sequence-to-sequence model. We evaluate our model using human-guided LLM labelling, which complements our reconstruction error in the translation from embedding and latents.
...
We test our *hypotheses* with qualitative and quantitative methods. _Does disentanglement occur?_ We use DCI and MIG to evaluate it. _Does the architecture outperform prompting?_ We compare our model using a human-guided classifier model: we manually label some samples as *baseline* \~100, and tweak distribution parameters of the model to obtain human-like responses. We fix the softmax probability values of the emotion classifier, treating them as ground truth and compare performance against a human baseline and prompted LLMs with different sizes. _Are all architecture components necessary?_ We verify it with ablations by varying the number of attention pooling heads, the size of the latent space and the depth of the VAE.
