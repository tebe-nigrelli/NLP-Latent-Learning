...
Transformer based Encoder-Decoder models are capable of performing a wide range of tasks by first mapping sequences to a lower-dimensional representation, before using the encoding to autoregressively continue a new sequence. The highly irregular geometry of embedding spaces results in complete, yet uninterpretable representation of information. In this work, we investigate whether such a pathological space can still be disentangled by training a variational autoencoder (VAE) to perform the task.
#grid(
  columns: (1fr, 1fr),
  column-gutter: 1em,

  [
    We place the VAE between the encoder and decoder, forcing it to map to an interpretable embedding space then back to the original encoding. We opted for a VAE because it allows for random sampling of new samples, while allowing to explicitly enforce strong inductive biases in the latent space through an loss contribution. We impose that the first dimensions of the interpretable latent space correspond to a desired labelling of the text sample, while enforcing low relative correlation in the remaining features. While the VAE is trained, the encoder-decoder model remains frozen: as backbone, we use the T5 model, as its unified text-to-text framing makes its latent space more general-purpose and better studied across a wider range of tasks.
