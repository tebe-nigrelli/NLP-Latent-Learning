...
#import "tracl-titlebox.typ": TITLEBOX-END-MARKER, affiliations-state, print-title-footnotes
...
#let maketitle(papertitle: none, authors: none, anonymous: false, titlebox-height: 2cm) = {
...
#let abstract(abs) = {
  set par(leading: linespacing, spacing: parspacing, first-line-indent: 0em, justify: true)

  context match-target(
    paged: align(center, box(width: 90%, [
      #text(12pt)[*Abstract*]
      #v(0.8em)
...
#let acl(
  ...
  set par(leading: linespacing, first-line-indent: 4mm, justify: true, spacing: parspacing)
  ...
  // lists and enums
  set list(indent: 1em)
  show list: set par(spacing: parspacing)

  set enum(indent: 1em)
  show enum: set par(spacing: parspacing)
